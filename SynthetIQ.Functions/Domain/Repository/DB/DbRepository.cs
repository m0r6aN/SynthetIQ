...
namespace SynthetIQ.Function.Domain.Repository.DB
{
    ...
    public sealed class DbRepository
    {
        ...
        public DbRepository(SynthetIQDbContext dbContext, AppInsightsLogger logger)
        {
        ...
        /// <summary>
        /// Fetches entities based on a specified condition. This is particularly useful for more complex queries that require filtering.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="condition"></param>
        /// <param name="ct"></param>
        /// <returns></returns>
        public async Task<IEnumerable<dynamic>> GetEntitiesByConditionAsync<T>(Expression<Func<T, bool>> condition, CancellationToken ct) where T : class
        {
            ct.ThrowIfCancellationRequested();
            return await SynthetIQDbContext.Set<T>().Where(condition).ToListAsync(ct);
        }

        /// <summary>
        /// Returns the count of entities that match a specific condition, useful for pagination or conditional logic within the application.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="condition"></param>
        /// <param name="ct"></param>
        /// <returns></returns>
        public async Task<int> CountEntitiesAsync<T>(Expression<Func<T, bool>> condition, CancellationToken ct) where T : class
        {
            ct.ThrowIfCancellationRequested();
            return await SynthetIQDbContext.Set<T>().CountAsync(condition, ct);
        }
